...
hrmsApp
    ...
    .directive("pageHeading", function() {
        return {
            restrict: 'E',
            transclude: true,
            template: '<div class="heading_bg toptext">' +
    ...
    .directive("subHeading", function() {
        return {
            restrict: 'E',
            transclude: true,
            template: '<div class="heading_h4"><ng-transclude></ng-transclude></div>'
        }
    })
    .directive("nonEditableInput", function() {
        return {
            restrict: 'E',
            scope: {},
            link: function($scope, $elem, $attrs) {
                $scope.IconShow = $attrs.icon == undefined ? false : true;
                $scope.iconData = $attrs.icon == '' ? 'people' : $attrs.icon;
                $scope.labelData = $attrs.label == undefined ? 'No label' : $attrs.label;
                $scope.valueData = $attrs.value == undefined ? 'No value' : $attrs.value;
            },
            template: '<div class="uk-input-group">' +
                '<span ng-if="IconShow" class="uk-input-group-addon">' +
                '<i class="material-icons">{{iconData}}</i>' +
                '</span>' +
                '<p class="inputLabel" for="companyname">{{labelData}}</p>' +
                '<p class="input_value">{{valueData}}</p>' +
                '</div>'
        }
    })


    //min number below 100
        ...
        link: function(scope, element, attributes) {
            ...
            element.on("keydown keyup", function(e) {
                ...
                if (Number(element.val()) > Number(attributes.max) && e.keyCode != 46  && e.keyCode != 8) 
                {
                    e.preventDefault();
                    element.val(oldVal);
                 //console.log(element.val(oldVal),"--oldVal1---")
